...
class Tests_front_end_sock_shop(unittest.TestCase):
    ...
    def test_133_get_(self, data_row):
        PHPSESSID, action, album, category, content, feed, id_, invitaion_code, mod, name, page, param, s, vars_0_, weekstartday = data_row

        # GET http://front-end.sock-shop/ (endp 133)
        front_end_sock_shop = get_http_client('http://front-end.sock-shop', authenticate)
        qstr = '?' + urlencode([('', param), ('\x00', ''), ("/'", ''), ('<script>alert(document.domain)</script>', ''), ('OpenServer', ''), ('PHPSESSID', PHPSESSID), ('XDEBUG_SESSION_START', 'phpstorm'), ('a', 'fetch'), ('action', action), ('album', album), ('category', category), ('content', content), ('cpmvc_do_action', 'mvparse'), ('data', '1'), ('f', 'edit'), ('feed', feed), ('filter', 'phpinfo'), ('function', 'call_user_func_array'), ('id', id_), ('invitaion_code', invitaion_code), ('mod', mod), ('mode', 'view'), ('name', name), ('op', 'browse'), ('page', page), ('parent', '0'), ('q[]', 'x'), ('s', s), ('show_dash_widget', '1'), ('vars[0]', vars_0_), ('weekstartday', weekstartday)])
        resp = front_end_sock_shop.get('/' + qstr)
        resp.assert_status_code(200)
